<template>
  <div class="max-w-3xl mx-auto">
    ...
    <div
      ...
      class="bg-white p-6 rounded-2xl shadow-sm mb-8 border border-slate-100 flex justify-between items-center"
    >
      <p class="text-lg text-slate-700 font-medium">
        Pozdrav, <span class="text-pink-500">{{ user.email }}</span
        >!
      </p>
    </div>
    ...
    <div
      v-if="myPets.length === 0"
      class="text-center py-10 bg-white rounded-2xl border border-dashed border-pink-200"
    >
...
<script>
import { getPlushieName } from "@/homeAssistant";
    ...
    if (this.isAuthenticated) {
      try {
        ...
        for (const pet of this.myPets) {
          getPlushieName(pet.pet_id).then((name) => (pet.pet_name = name));
        }
      } catch (error) {
