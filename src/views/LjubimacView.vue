<template>
  <div
    class="max-w-2xl mx-auto py-8 text-center bg-white p-8 rounded-3xl shadow-lg border border-pink-50"
  >
    <div
      v-if="loading"
      class="animate-pulse flex flex-col items-center space-y-4"
    >
    ...
    <div v-else class="flex flex-col items-center">
      <h1 class="text-4xl font-extrabold text-pink-500 mb-2">
        Ljubimac {{ name }}
      </h1>
      ...
      <div class="relative inline-block mb-8 group">
        <div
          class="absolute -inset-1 bg-gradient-to-r from-pink-300 to-purple-300 rounded-full blur opacity-25 group-hover:opacity-50 transition duration-1000 group-hover:duration-200"
        ></div>
        <img
          src="/images/medo.png"
          class="w-48 h-48 object-contain relative mx-auto drop-shadow-xl transform hover:scale-105 transition-transform duration-300"
          alt="Medo"
        />
      </div>

      <div
        class="mb-8 w-full max-w-xs mx-auto bg-slate-50 p-4 rounded-2xl shadow-inner"
      >
      ...
      <div
        v-if="editingName"
        class="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-3 mb-6 bg-pink-50 p-4 rounded-xl w-full max-w-md"
      >
        <input
          v-model="newName"
        ...
        <div class="flex space-x-2">
          <button
            @click="saveName"
            class="bg-pink-500 text-white px-4 py-2 rounded-xl font-medium hover:bg-pink-600 transition-colors shadow-sm"
          >
            Spremi
          </button>
          <button
            @click="editingName = false"
            class="bg-white text-slate-600 px-4 py-2 rounded-xl font-medium border border-slate-200 hover:bg-slate-50 transition-colors"
          >
            Odustani
          </button>
        </div>
      ...
      <div v-else class="mb-6">
        <button
          @click="startEditName"
          class="text-sm text-pink-600 hover:text-pink-700 underline font-medium underline-offset-4"
        >
          Promijeni ime
        </button>
      </div>
      ...
      <div class="flex flex-col sm:flex-row gap-4 w-full justify-center">
        ...
        <button
          v-if="
            isAuthenticated &&
            !petsLoading &&
            !myPets.some((pet) => pet.pet_id === $route.params.id)
          "
          @click="addPet"
...
export default {
  ...
  methods: {
    ...
    async saveName() {
      ...
      await setPlushieName(Number(this.$route.params.id), this.newName.trim());
    ...
    async pricaj() {
      await playSound(Number(this.$route.params.id), this.happiness);
    },
    async addPet() {
      try {
        const token = await this.getAccessTokenSilently();
